...
[[maybe_unused]] LinearProgram::LinearProgram(Column target_vector, Matrix constraints_matrix,
                                              Column constraints_vector) : _target_vector(std::move(target_vector)),
                                                                           _constraints_vector(
                                                                                   std::move(constraints_vector)),
                                                                           _constraints_matrix(
                                                                                   std::move(constraints_matrix)) {
    _state = UNSOLVED;
...
void LinearProgram::set_solution(Column soluiton) {
    assert(verify_solution(soluiton));
    _solution = std::move(soluiton);
}

bool LinearProgram::verify_solution(const Column &solution) const {
...
Column LinearProgram::solution() const {
    return _solution;
}

Value LinearProgram::solution_value() const {
